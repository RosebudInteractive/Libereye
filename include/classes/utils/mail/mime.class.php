...
class Mail_Mime
{
     ...
     * @param string $encoding text encoding (7biy, quote-printable etc.)
     */
    function setTextEncoding($encoding = 'Base64')
    {
     ...
     * @param string $charset charset for text (utf-8 by default)
     */
     ...
     * @param string $charset charset for HTML (utf-8 by default)
     */
     ...
     * @param int $count number of symbols to wrap
     */
     ...
     * @param string $html       html code of message
     * @param string $text       text variant of message
     * @param string $images_dir directory with images
     */
    function setHtml($html, $text = null, $images_dir = null)
    {
        $this->html      = $html;
        $this->html_text = $text;
        $images_dir = $images_dir ? $images_dir: $this->images_dir;
     ...
     * by add_html() and find inside html-tags any file that
     * ends in one of the extensions defined in $obj->image_types.
    ...
    function _findHtmlImages($images_dir)
    {
        ...
        preg_match_all('/(?:\<[^\>]+src=\"|\')([^\"\']+\.('.implode('|', $extensions).'))(?:\"|\'[^\>]?\>)/Ui', $this->html, $images);
        ...
        if (!empty($html_images)) {
            ...
            for ($i=0; $i<count($html_images); $i++) {
                if ($image = file_get_contents($images_dir.$html_images[$i])) {
    ...
    function &_addTextPart(&$obj, $text)
    {
        ...
        if (is_object($obj))
        $oPart = &$obj->addSubpart($text, $params);
        else
        $oPart = new Mail_mimePart($text, $params);
    ...
    function &_addHtmlPart(&$obj)
    {
        ...
        if (is_object($obj)) {
            $oPart = &$obj->addSubpart($this->html, $params);
        } else {
            $oPart = new Mail_mimePart($this->html, $params);
        }
        return $oPart;
    }
    ...
    function &_addMixedPart()
    {
        ...
        $oPart = new Mail_mimePart('', $params);
        return $oPart;
    }
     ...
     *   html_encoding - The type of encoding to use on html. Valid options are
     *                   "Base64", "quoted-printable" or "base64" (all without quotes).
     *                   Base64 is EXPRESSLY NOT RECOMMENDED. Default is quoted-printable
     *   text_encoding - The type of encoding to use on plain text Valid options are
     *                   "Base64", "quoted-printable" or "base64" (all without quotes).
     *                   Default is Base64
     *   text_wrap     - The character count at which to wrap Base64 encoded data.
     *                   Default this is 998.
     *   html_charset  - The character set to use for a html section.
     *                   Default is utf-8
     *   text_charset  - The character set to use for a text section.
     *                   Default is utf-8
     *   head_charset  - The character set to use for header encoding should it be needed.
     *                   Default is utf-8
     */
    ...
    function send($recipients, $type = 'mail')
    {
        ...
        switch ($type) {
            ...
            case 'smtp':
                require_once 'classes/utils/mail/smtp.class.php';
