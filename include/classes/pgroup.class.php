...
class Pgroup extends DbItem
{

    function Pgroup()
    {
        parent::DbItem();
        $this->_initTable('pgroup');
        $this->nObjectType = 7;
    }
    ...
    function getList($aCond=array(), $iPage=0, $iPageSize=0, $sSort='', $aFields=array(), $nLangId=0)
    {
        ...
        if ($iCnt)
        {
            ...
            $sSql = 'SELECT '.$this->_joinFields($aMap, $aFields). ', i.name image, count(*) ptypes'.
                    ', pd1.phrase title'.
                    ' FROM '.$this->sTable.' AS '.$this->sAlias.
                    ' LEFT JOIN image i ON pg.image_id=i.image_id'.
                    ' LEFT JOIN phrase p1 ON p1.object_id='.$this->sAlias.'.pgroup_id AND p1.object_type_id='.$this->nObjectType.' AND p1.object_field="title" '.
                    ' LEFT JOIN phrase_det pd1 ON pd1.phrase_id=p1.phrase_id AND pd1.language_id='.$nLangId.'  '.
                    ' LEFT JOIN ptype2group p2g ON p2g.pgroup_id='.$this->sAlias.'.pgroup_id'.
                    ' WHERE  '.$sCond.
                    ' GROUP BY p2g.pgroup_id'.
    ...
    function getListOffset($aCond=array(), $iOffset=0, $iPageSize=0, $sSort='', $aFields=array(), $nLangId=0)
    {
        ...
        if ($iCnt)
        {
            $sSql = 'SELECT '.$this->_joinFields($aMap, $aFields).', i.name image'.
                    ', pd1.phrase title'.
                    ' FROM '.$this->sTable.' AS '.$this->sAlias.
                    ' LEFT JOIN image i ON pg.image_id=b.image_id'.
